...
// WebApplicationBuilder automatically registers several configuration providers (IConfigurationProvider implementations) by default; these support loading settings from appsettings.json and environment-specific variations thereof, user secrets, environment variables, and command line arguments.
...
// The default providers are configured such that their priority isn't strictly linear, i.e., the last provider read always wins out; e.g., command-line arguments are registered twice, once at the beginning and at the end; certain environment variables have higher priority than others and from settings from other providers. See https://learn.microsoft.com/en-us/aspnet/core/fundamentals/configuration/?view=aspnetcore-10.0#default-app-configuration-sources
