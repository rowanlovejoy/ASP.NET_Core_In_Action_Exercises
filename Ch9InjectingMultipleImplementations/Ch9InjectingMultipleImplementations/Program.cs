...
// The book says that to replace a previously registered implementation, the Replace call must specify the same lifetime as the implementation to be replaced; e.g., a scoped implementation can be replaced only if with another scoped implementation. However, this doesn't appear to be case, at least in ASP.NET 10. Here, I'm replacing a scoped implementation with a singleton, and the replacement is occurring as expected. Perhaps this was a restriction present in ASP.NET 7, the subject of the book, but that has since been removed.
